		...
		<div class="modal-dialog modal-lg" role="document">
			<div class="modal-content">
				...
				<div class="modal-footer">
					<div>
						<div id="modeEQ" class="btn-group dropdown dropSelect" style="margin-top:10px; margin-bottom:8px;">
							<button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown"> <span>自定义配置</span> <span class="caret"></span> </button>
							<ul class="dropdown-menu" role="menu">
								<li class="active" value="eq0.json"><a href="javascript:void(0);">自定义配置</a>
								</li>
								<li value="eq1.json"><a href="javascript:void(0);">男声配置</a>
								</li>
								<li value="eq2.json"><a href="javascript:void(0);">女声配置</a>
								</li>
							</ul>
						</div>
						<button class="btn btn btn-danger" id="saveEQ">保存</button>
						<button class="btn btn btn-default" id="cancelEQ">取消设置</button>
					</div>
				</div>
			</div>
		</div>
					...
					<div class="panel-body dark">
						<div class="row text-center">
							<div class="col">
								...
								<div class="volume nogain" id="vol_remote" min="0" max="100" percent="true"></div>
								<div class="nomute"></div>
							</div>
...
<div class="row">
	<div class="col-lg-12">
		<div class="panel panel-default">
			...
			<div class=" panel-body text-center">
				...
				<div>
					<div id="mode" class="btn-group dropdown dropSelect" style="margin-top:10px; margin-bottom:8px;">
						<button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown"> <span>配置方案1</span> <span class="caret"></span> </button>
						<ul class="dropdown-menu" role="menu">
							<li class="active" value="set0.json"><a href="javascript:void(0);">配置方案1</a>
							</li>
							<li value="set1.json"><a href="javascript:void(0);">配置方案2</a>
							</li>
							<li value="set2.json"><a href="javascript:void(0);">配置方案3</a>
							</li>
							<li value="set3.json"><a href="javascript:void(0);">配置方案4</a>
							</li>
							<li value="set4.json"><a href="javascript:void(0);">配置方案5</a>
							</li>
							<li value="set5.json"><a href="javascript:void(0);">配置方案6</a>
							</li>
						</ul>
					</div>
					<button class="btn btn btn-danger" id="save">保存</button>
					<button class="btn btn btn-default" id="cancel">取消设置</button>
					<button class="btn btn btn-default" id="default">还原默认</button>
				</div>
			</div>
		</div>
	</div>
	<div class="col-lg-12">
		<div class="well">
			<strong>操作提示：</strong>
			<ul>
				<li>滑动条表示增益或减溢的幅度，不代表音量的绝对值范围，与计算机音量调节不同。因此，即使拉到底部也不代表静音，静音请点击 <span class="glyphicon glyphicon-volume-up"></span> 图标。</li>
				<li>本页面任何操作立即生效，但需要点击保存按钮才会将设置储存到系统中。</li>
				<li>每次开机自动调用最后一次使用的配置方案</li>
			</ul>
		</div>
	</div>
</div>
...
<script>
	...
	$( ".mute" ).append( $( "<span class='glyphicon glyphicon-volume-off'></span><span class='glyphicon glyphicon-volume-up'></span>" ) );
	...
	function getAudio() {
		rpc( "getAudio", [], function ( data ) {
			...
			$( "#output #vol_monitor" ).gain( output.gain1, true );
			$( "#output #mute_output" ).mute( output.mute0 );
			$( "#output #mute_monitor" ).mute( output.mute1 );
	...
	function setAudio() {
		...
		output.gain1 = $( "#output #vol_monitor" ).gain();
		output.mute0 = $( "#output #mute_output" ).mute();
		output.mute1 = $( "#output #mute_monitor" ).mute();
		...
		rpc( "setAudio", [ data ], function ( data ) {
			if ( typeof ( data.error ) != "undefined" ) {
				$( "#audio" ).myAlert( "danger", "通信错误:", data.error );
				return;
			}
